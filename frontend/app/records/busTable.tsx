...
import type { BusData } from "./records";
...
export function BusTable() {
    ...
    const { data: busData = [], isLoading, refetch } = useQuery<BusData[]>({
        queryKey: ['busData'],
        queryFn: async () => await get(`${import.meta.env.VITE_BACKEND_API_URL}/v1/bus/records`),
    });

    const filteredData = useMemo(() => busData.filter(item => {
        const searchTerm = filters.search.toLowerCase();
        const matchesSearch = filters.search ? [
            item.lgu, item.barangay, item.hhId, item.granteeName, item.typeOfUpdate,
            item.subjectOfChange, item.issue || '', item.date
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => refetch()}
                            className="inline-flex items-center px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-200 shadow-sm"
                            disabled={isLoading}
                        >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="flex items-end">
                                <button
                                    ...
                                    className="w-full px-3 py-2 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg transition-colors duration-200"
                                >
                                    Clear Filters
                                </button>
                            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">LGU</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Barangay</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">HH ID</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Grantee Name</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Update Type</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Subject of Change</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Issue</th>
                            ...
                            {isLoading ? (
                                ...
                                paginatedData.map((item, index) => (
                                    <tr key={item.id} className={`hover:bg-gray-50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                                        <td className="px-3 py-4 text-sm text-gray-900">{item.lgu}</td>
                                        <td className="px-3 py-4 text-sm text-gray-900">{item.barangay}</td>
                                        ...
                                        <td className="px-3 py-4 text-sm text-gray-900">{item.granteeName}</td>
                                        <td className="px-2 py-4 text-sm text-gray-900">{item.typeOfUpdate}</td>
                                        <td className="px-3 py-4 text-sm text-gray-900">{item.subjectOfChange}</td>
                                ...
                                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    ...
                                    {[...Array(totalPages)].map((_, i) => {
                                        ...
                                        if (
                                            ...
                                            return (
                                                <span key={pageNum} className="relative inline-flex items-center px-3 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700">
                                                    ...
                                                </span>
                                            );
                                        }
